    ...
    return (
        <div className="row m-2 d-flex justify-content-center">
            <form className="col-4" onSubmit={async (e)=>{
                ...
                setload(true)
                setvis("hidden")
                let msg = await axios.post('https://url-back.herokuapp.com/register',{user_Id,name,password,links})
                alert(msg.data.message)
                redirect.push('/')
                }}>
                <label htmlFor="userid"> User ID : <span className={text_color}>{comment}</span></label>
                <input required className="form-control" id="userid" value={user_Id} onChange={(e)=>{
